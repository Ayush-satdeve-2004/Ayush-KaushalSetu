...
export default function AuthScreen({ onLoginSuccess, onClose }) {
  ...
  const quickLogin = (emailStr) => {
    setLoginEmail(emailStr);
    setLoginPassword('Pass1234!');
    fetch(`${API_BASE}/auth/login`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: emailStr, password: 'Pass1234!' })
    })
      .then(res => res.json())
      .then(json => {
        if (json.success && json.user) {
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col justify-center items-center p-4 relative overflow-hidden font-sans">
      ...
      <div className="relative z-10 max-w-md w-full bg-white rounded-3xl shadow-2xl overflow-hidden border border-slate-200 my-6 flex flex-col max-h-[90vh]">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-lg font-bold leading-tight">
                {mode === 'forgot' ? 'Reset Password' : mode === 'register' ? 'Register Account' : 'AYUSH KaushalSetu'}
              </h2>
          ...
          {mode === 'login' ? (
            ...
            <form onSubmit={handleLoginSubmit} className="space-y-4 text-xs">
              ...
              <button
                ...
                className="w-full bg-emerald-700 hover:bg-emerald-800 text-white font-bold py-3 rounded-xl transition-all shadow-md flex items-center justify-center space-x-2 text-sm"
              >
                <span>{loginLoading ? 'Signing In...' : 'Sign In to Your Role Portal'}</span>
              ...
              <div className="pt-3 border-t border-slate-100">
                <p className="text-[11px] font-bold text-slate-500 text-center mb-2">
                  Or 1-Click Demo Login to Any Portal:
                </p>
                <div className="grid grid-cols-2 gap-1.5">
                  <button
                    type="button"
                    onClick={() => quickLogin('aarav.sharma@student.aiia.ac.in')}
                    className="bg-slate-100 hover:bg-slate-200 text-slate-700 text-[10px] font-bold py-1.5 px-2 rounded-lg text-left truncate"
                  >
                    🎓 Student Portal (Aarav)
                  </button>
                  <button
                    type="button"
                    onClick={() => quickLogin('recruitment@daburayush.com')}
                    className="bg-slate-100 hover:bg-slate-200 text-slate-700 text-[10px] font-bold py-1.5 px-2 rounded-lg text-left truncate"
                  >
                    🏢 Industry Portal (Dabur)
                  </button>
                  <button
                    type="button"
                    onClick={() => quickLogin('academic@aiia.gov.in')}
                    className="bg-slate-100 hover:bg-slate-200 text-slate-700 text-[10px] font-bold py-1.5 px-2 rounded-lg text-left truncate"
                  >
                    🏫 Institution Portal (AIIA)
                  </button>
                  <button
                    type="button"
                    onClick={() => quickLogin('certification@swayam-ayush.in')}
                    className="bg-slate-100 hover:bg-slate-200 text-slate-700 text-[10px] font-bold py-1.5 px-2 rounded-lg text-left truncate"
                  >
                    🌐 Skill Platform (Swayam)
                  </button>
                </div>
              </div>
